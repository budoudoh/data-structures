/*
    ...
    list has been scrambled. We just want to know what the lowest rated flavor of oreos is so that 
    we can avoid that flavor like the plague. 
    ...
    following:
        ...
        lowest rated flavor efficiently
        - iterate through the data structure and return the name of the lowest rated oreo
    ...
    structure. Finally, use the function "findLowestRatedOreo" to define the logic you want to you with your 
    data structure to find the lowest rated oreo flavor in the rating list. When you 
    are ready to run your code, navigate the console to the folder that holds this file
    ("cd non-linear"), then type the commands "npm install" and "npm run problem3" into the command line on 
...
fs.createReadStream(path.resolve(__dirname, 'assets', '../data/oreo_rankings.csv'))
    ...
    .on('end', (rowCount) => findLowestRatedOreo(my_data_structure));


function findLowestRatedOreo(structure){
    let oreo_flavor;
    /* 
